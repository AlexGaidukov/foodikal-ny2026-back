...
async def handle_get_menu(db: Database) -> Response:
    """
    GET /api/menu
    ...
    try:
        ...
        return json_response(grouped_menu)
...
async def handle_get_menu_by_category(db: Database, category_encoded: str) -> Response:
    """
    ...
    try:
        ...
        return error_response("Failed to fetch menu category", 500)


async def handle_create_order(request, db: Database, telegram_bot_token: str, telegram_chat_id: str, environment: str) -> Response:
    """
    ...
    try:
        ...
        if not is_valid:
            return error_response("Invalid order data", 400, details=errors)
        ...
        return error_response(e.message, 400, details=e.details)
...
async def handle_get_orders(db: Database) -> Response:
    """
...
async def handle_update_order(request, db: Database, order_id: int) -> Response:
    """
...
async def handle_delete_order(db: Database, order_id: int) -> Response:
    """
...
async def handle_add_menu_item(request, db: Database) -> Response:
    """
    ...
    try:
        ...
        if not is_valid:
            return error_response("Invalid menu item data", 400, details=errors)
...
async def handle_update_menu_item(request, db: Database, item_id: int) -> Response:
    """
...
async def handle_delete_menu_item(db: Database, item_id: int) -> Response:
    """
    ...
    try:
        ...
        return error_response("Failed to delete menu item", 500)
...
async def route_request(request, env) -> Response:
    ...
    if method == "GET" and "/api/menu/category/" in url:
        ...
        return await handle_get_menu_by_category(db, category)

    if method == "GET" and url.endswith("/api/menu"):
        return await handle_get_menu(db)
